...
left_join(avg, compare, by = 'month') %>% 
  ...
  geom_ribbon(aes(ymin = pmin(0, difference) + compare_avg, ymax = compare_avg, fill = "black", alpha = 0.5, group = 1)) +
...
left_join(avg, compare, by = 'week') %>% 
  ...
  geom_line(aes(y = diff, group = 1), color = 'black', lwd = 1.3) +
