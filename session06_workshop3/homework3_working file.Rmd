...
> We will analyse the weight variable. To start off, we can see from the skim table that there's 1004 missing values in the column. The distribution of the remaining values can be seen in the histogram below. The distribution is skewed right, with a mean of 67.91 kilograms.
...
> 
>>>>>>> 8f502fa86b22ef3d33f3bcee31ee60007cd8b701
...
<<<<<<< HEAD
=======
> We have calculated a 95% confdence interval for the proportion of high schoolers who are not active 3 or more days a week, which is 0.3 - 0.326. This means the majority of people are active 3+ days a week. 
...
> We would have expected physical activity to be inversely correlated with weight, as activity should make you lose weight. However, it appears in the boxplot that this is not the case. We could consider that muscle is heavier than fat, and people who exercise often have more muscle.
...
<<<<<<< HEAD
=======
> According to the t-test, there is a signifcant difference between the weight of people with 3+ days of phsyical activity and those without. Those who have 3+ days of physical activity have a higher weight.
...
Our null hypothesis is that the means of the ratings of their films is the same.
The alternative hypothesis is that there is a difference between the means of the ratings of Tim Burtons and Steven Spielbergs films.
We will be using the t-statistic for the difference between the means. We are looking for a p-value smaller than 0.05 or a t-statistic bigger than 1.75 or smaller than -1.75 (based on df = 15 and p = 0.05). We have chosen the degrees of freedom as the smallest count - 1, which was 16 for Tim Burton.
...
ggplot(omega)+
  aes(x = salary, y = experience, color = gender)+
  geom_point() +
  geom_smooth()
...
ggplot(filtered_brexit_data) + 
  ...
  scale_color_manual(breaks = c("con_2015", "lab_2015", "ld_2015", "ukip_2015"), values=c("#0087DC", "#E4003B", "#FAA61A", "#6D3177")) +
