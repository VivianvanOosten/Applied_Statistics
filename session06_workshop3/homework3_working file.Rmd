...
The data frame `omega` contains the salaries for the sample of 50 executives in the company. We have constructed a visualization that shows the salary trend between male and female employees:
...
We have GDP data from UN over the years 1970-2016 and are going to analyse the components of GDP over that time to see if the theory that the sum of all indicators equals GDP corresponds to reality. Below we download the data and see what it looks like.
...
We tidy the data by converting it to a long format. We need to change the Indicator Names to be more concise indicators to ensure legibility in the plots.To test our hypothesis that the GDP theory is correct, we plot the GDP data by it's components for Germany, India and United States of America.

```{r reshape_GDP_data, eval=TRUE}
...
drop_na(tidy_GDP_data) 
tidy_GDP_data


#et us compare GDP components for these 3 countries
...
We calculate the GDP calculated from it's components and compare that to the actual GDP. To see how much it differs from each other, we calculate the percentage difference.
...
#summing the total gdp reported (separate from components)  
...
#with(tidy_GDP_data_countrywise, sum(GDP_new[Country == ]))
```
