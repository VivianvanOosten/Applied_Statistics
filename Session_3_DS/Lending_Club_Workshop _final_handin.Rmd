...
# Lending club
We have been tasked to undertake an analysis on data provided about the Lending Club, a personal loan and full-service banking financial service provider in the United States of America. The data provided to us contains information related to both the loan and the borrowers’ personal details. The team conducted analysis that followed EDA, and tests using OLS, Lasso, and Ridge statistic models to decipher which predictive model can most accurately predict interest rates for loans.
...
We first take the data and clean it up. 
...
ggplot(lc_clean) +
  aes(x = annual_inc, y = int_rate) +
  geom_point()+
  geom_smooth(method = 'lm', se = FALSE)+
  labs(
    title = 'Interest Rate increases with Increase in income',
    subtitle = 'Relationship between annual income and interest rate',
    x = 'Annual Income',
    y = 'Interest Rate'
  ) + 
...
To set a baseline for our algorithm, we start with a simple linear regression with the following variables "term", "annual_inc", "dti", and "grade".
...
>  All variables are significant, as the p-value for all variables is below 0.05. 

b.  How much explanatory power does the model have?

>  Around 91.91% of variance in the interest rate is explained by our current model (the adjusted r-squared). The 95% confidence interval around any prediction from this model is 0.041552.



We have fit a new linear regression that is identical to model 0, however, with the extra variable "loan_amnt".
...
To find out which features belong in our model, we gradually build out our model further. 
...
model3 <-  lm(int_rate ~ term + dti+loan_amnt*grade + poly(annual_inc,3), data = lc_clean)
summary(model3)
...
model4 <- lm(int_rate ~ term + quartiles_annual_inc + dti + grade*loan_amnt, data = lc_clean
  )
summary(model4)
...
> a.) Model 4 has the most explanatory power. The explanatory variables are able to explain 92% of the variability in data. We have quartiled annual income, which helps us to explain how income for different quartiles can influence interest rate. The models have the same r-squared and RMSE but we chose model 4 because we beleive it is the most robust.

b.In model 1, how do you interpret the estimated coefficient of the interaction term between grade C and loan amount?
> b.) If the loan is graded as "C", its interest rate will be 6.20 % greatert than the loan graded as "A". If the loan amount is increases by 1 then interest rate is expected to increase by 0.000016154%.
...
> c.) Yes we must take into account the problem of multicollinearity. Multicollinearity can affect the independency of independent variables. The model is unable to predict relationship between independent variables. Including highly correlated explanatory variables affects the accuracy of our model.
...
# **Q5.** How much does the predictive accuracy of Model 2 deteriorate when we move from in-sample to out-of-sample testing? Is this sensitive to the random seed chosen? Is there any evidence of overfitting? {.unnumbered}

>When the seed is set to 23, the predictive accuracy of Model 2 deteriorates by 0.8864% when we move from in-sample to out-of-sample testing. This result is sensitive to the random seed chosen, as this value affects the way in which data is split into the training and testing datasets. Since the difference is minor, there is no evidence of overfitting - the model performs comparatively well when used for prediction based on external data.

# k-fold cross validation

We can also do out of sample testing using the method of k-fold cross validation which has several advantages over the traditional hold-out method.
...
# **Q7.** Using the learning curves above, approximately how large of a sample size would we need to estimate model 3 reliably? Once we reach this sample size, if we want to reduce the prediction error further what options do we have? {.unnumbered}

> Out-of-sample error will reduce as sample size increases to about 3000 data points. After that it increases which can be attributed to overfitting problems. Therefore 3000 is the sample size large enough to estimate model 3 reliably. We can further reduce prediction error by using more explanatory variable which can help us get a better fit eg we can use more data from the web or we can make the data in a wide format, or we can use other regression models and measure how they fit compared to our model.
...
>a) Having compared the out-of-sample performance of the OLS regression vs. the LASSO regression, we notice that the latter performs significantly better. This is because the small simple size (1% of the dataset) does not allow us to train OLS regressions properly and results in a poor model. On the other hand, LASSO performs better because it can single out significant coefficients (even on small smaples).
...
d. Why is it important to standardize continuous variables before running LAS?
>d) In using penalised regressions, it is importat to standardize all variables to ensure the penalisation is applied equally to all predictors in terms of unit variance.
...
>a) LASSO performs best out of sample out of the three models since it is generally preferred for problems where the sample size is small and the number of predictors is large. As this is the case here, it is expected that LASSO will yield the best results.

b. What value of lambda offers best performance?
>b) The value of lambda that offers best performance is 0.001581582. 
...
>c) In the Ridge regression model none of the coefficients are zero. This is because this method can only decrease the coefficients asimptoticallty to 0 but will never make them equal to 0, even if they are very insignificant. On the other hand, LASSO can shrink the coefficients to 0, which is why it is preferred where there are a lot of insignificant variables
...
>d) Standardization is equally important in Ridge regression as the penalty should be applied equally to all coefficients, regardless of unit of measurement. 
...
>e) One possible advantage of Ridge over OLS would be the case where we would like to create a model for prediction with a lot of predictor variables, most of which are significant. Ridge would yield the best results even if there are multicollinerity issues and would include all of the variables in the final model.
...
# **Q10.** Based on your analysis above, is there any evidence to suggest that interest rates change over time? Does including time trends / quarter-year dummies improve predictions? Any improvement in prediction performance? {.unnumbered}
...
bond_prices<-readr::read_csv("/Users/samarth/Desktop/Applied_Statistics/Session_3_DS/MonthBondYields .csv")
...
# **Q11.** Do bond yields have any explanatory power? Do not forget to interpret coefficients. {.unnumbered}
> Yes bond yield have an explanatory power and every variable is significant. To interpret the coefficients, when price increase by $1, the interest rate is expected to decrease 0.57%. If it is rated as B, the interest rate is expected to be 3.68% higher than the bond graded as A. For grade C, interest rate suppose to be 6.18% higher than grade A. For grade D, interest rate suppose to be 8.38% higher than grade A. For grade E, interest rate suppose to be 10.33% higher than grade A. For grade F, interest rate suppose to be 12.36% higher than grade A. For grade C, interest rate suppose to be 14.01% higher than grade A
> 

# **Q12.** Choose a model and describe your methodology {.unnumbered}
...
# **Q13.** (optional) Use other publicly available datasets to further improve performance (e.g., quarterly data on US inflation, [CPI](https://fred.stlouisfed.org/series/CPALTT01USQ657N), [unemployment rate](https://fred.stlouisfed.org/series/UNRATE)). Explain why you think the additional data will make a difference and check if it does. {.unnumbered}

> We expect macroeconomic data (CPI and unemployment) to improve the performance of our best model (Model4) as interest rates depend heavily on macroeconomic indicators. Thus, we will join publicly avilable datasets with our current data to determine if it helps us proedict interest rates more accurately. Then, we will extend model4 with two additional variables and see if the performance improves significantly.

```{r additional data}
cpi <- read_csv("CPALTT01USQ657N.csv") %>% 
  mutate(quarter = as.factor(as.yearqtr(DATE, format = "%Y-%m-%d")))
  unemploy <- read_csv("UNRATE.csv")

lc_cpi <- lc_clean %>% 
  mutate(quarter = as.factor(as.yearqtr(issue_d, format = "%Y-%m-%d"))) %>% 
  left_join(cpi, by="quarter")

lc_macro <- lc_cpi %>% 
  left_join(unemploy, by=c("issue_d" = "DATE")) %>% 
  arrange(issue_d)

names(lc_macro)[24:25] <- c("cpi", "unemployment")
  
lc_macro_clean <- lc_macro %>% 
  mutate(cpi = as.double(cpi))

glimpse(lc_macro)

model4 <- lm( int_rate ~ quartiles_annual_inc  + term  + dti + grade*loan_amnt , data = lc_clean)
model4_extended <- lm( int_rate ~ quartiles_annual_inc  + term  + dti + grade*loan_amnt + cpi + unemployment, data = lc_macro_clean)
summary(model4)
summary(model4_extended)


```


> From the output above it is noticeable that both of the explanatory variables we have added (CPI and Unemployment Rate) are significant predictors of interest rates. However, they do not add much explanatory power to the model - R-squared only increased from 0.9204 to 0.9251 between model4 and the extended version. This is likely because the original model already had high explanatory power and most of the variability in interest rates was already explained by the previously included variables. Hence, it is best to omit CPI and Unemployment Rate from the model as they make the model more complex and do not necessarily help predict interest rates more accurately.
