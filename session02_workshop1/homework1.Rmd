...
> No missing values within any of the columns as inspected using the Skimr package.
> When checking for duplicates within the dataset, we identifies that there were 54 duplicated titles which may result in duplicated entries within the dataset.
...
#Default Table 
#FIX THIS
c <- ggplot(data) + aes(x = mean, y = genre ) + geom_col() + labs(
    title = 'San Fransisco accounts for more than a quarter of all rental classifieds',
    subtitle = '% of Craigslist listings, 2000-2018',
    x = NULL,
    y = NULL,
...
#Answer
ANSWER: The number of facebook likes is not a good predictor of how much money a movie will make at the box office as movies with the same number of likes received by cast earned vastly different amounts.
...
ANSWER: Gross US Earnings could be predicted by looking at the movie's budget as the two variables display a positive relationship relationship. 
...
ANSWER: In general, IMDB ratings could be used to predict the gross earnings of movies in the US - this positive relationship is particularly visible in the Action, Adventure, and Comedy genres. However, data in the 'movies' dataset is not uniformly distributed across genres, with some genres containing only a few data points, which prohibits a meaningful analysis of the relationship between IMDB ratings and gross earnings. 
